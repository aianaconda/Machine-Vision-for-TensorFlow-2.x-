...
import os
# 载入本工程的代码模块
from code_20_CRNNModel import ctc_lambda_func
from code_19_mydataset import get_dataset, decodeindex
from code_24_STNCRNNModel import STNCRNN
...
output_dir = 'resultSTNCRNN'  # 定义模型的输出路径
...
# 定义模型
STNCRNN_model = STNCRNN(model_config)
STNCRNN_model.summary()  # 输出模型信息
...
if output_File is not None:
    ...
    STNCRNN_model.load_weights(output_File)
...
STNCRNN_model.compile(loss=myctc, optimizer=optimizer, target_tensors=[input_labels])

# 训练模型，迭代400次
STNCRNN_model.fit(image_dataset, steps_per_epoch=steps_per_epoch, epochs=400, verbose=1,
...
# 定义CTC解码运算模型
input_pred = Input(STNCRNN_model.output.shape[1:])
...
for i, j in image_dataset.take(1):  # 取出一个批次的数据
    y_pred = STNCRNN_model.predict(i, steps=1)  # 将数据送入模型中进行预测
    #        print(y_pred)
